    ...
    <div className="min-h-screen bg-[#F1F4F9] text-slate-900 font-sans selection:bg-indigo-100 print:bg-white">
      ...
      <nav className="bg-white border-b border-slate-200 px-8 py-4 flex items-center justify-between sticky top-0 z-50 print:hidden">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="font-black text-slate-800 leading-none">PsicoAssistente</h1>
        ...
        <div className="min-h-[600px]">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-48">
              ...
              <h2 className="text-2xl font-black text-slate-800">Processando Inteligência</h2>
    ...
    <div className="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/50 p-12 md:p-16 space-y-12 animate-in fade-in zoom-in-95 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="space-y-3">
          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Profissão do Emissor</label>
          <select 
        ...
        <div className="space-y-3">
          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Finalidade do Relatório</label>
          <select 
            className="w-full h-16 px-6 rounded-2xl bg-slate-50 border-2 border-slate-100 font-bold text-slate-700 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-50 outline-none transition-all appearance-none"
  ...
  return (
    <div className="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/50 p-12 md:p-16 space-y-12 animate-in fade-in slide-in-from-right-8 duration-500">
      <div className="flex justify-between items-start">
        <div className="space-y-2">
          <h2 className="text-4xl font-black text-slate-950 tracking-tight">Entrada de Dados</h2>
          <p className="text-slate-400 text-lg font-medium">Extraímos automaticamente os resultados para você.</p>
        </div>
      ...
      <div className="flex bg-slate-100 p-1.5 rounded-2xl w-fit border border-slate-200">
        <button onClick={() => setMode('text')} className={`px-10 py-3 rounded-xl font-black text-sm transition-all ${mode === 'text' ? 'bg-white shadow-md text-indigo-600' : 'text-slate-400'}`}>COLAR TEXTO</button>
        <button onClick={() => setMode('image')} className={`px-10 py-3 rounded-xl font-black text-sm transition-all ${mode === 'image' ? 'bg-white shadow-md text-indigo-600' : 'text-slate-400'}`}>ENVIAR PRINT</button>
      </div>

      {mode === 'text' ? (
        <textarea 
          placeholder="Cole aqui os scores e classificações das suas planilhas..."
        ...
        <div className="group relative h-80 border-4 border-dashed border-slate-200 rounded-[2.5rem] hover:bg-indigo-50/50 hover:border-indigo-200 transition-all flex flex-col items-center justify-center cursor-pointer">
          ...
          <p className="font-black text-xl text-slate-900">Arraste ou clique para enviar</p>
          <p className="text-sm text-slate-400 font-bold uppercase mt-2">OCR Clínico Habilitado</p>
        </div>
      )}
    ...
    <div className="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/50 p-12 md:p-16 space-y-12 animate-in fade-in slide-in-from-right-8 duration-500">
      <div className="flex justify-between items-start">
        <div className="space-y-2">
          ...
          <p className="text-slate-400 text-lg font-medium">Revise as métricas antes da redação canônica.</p>
        </div>
        <button onClick={onBack} className="text-xs font-black text-indigo-600 uppercase tracking-widest border-b-2 border-indigo-600 pb-1">Corrigir Dados</button>
      </div>
        ...
        {data.map((item, idx) => (
          <div key={idx} className="p-8 rounded-[2.5rem] bg-slate-50 border border-slate-100">
            <div className="flex justify-between items-center mb-6">
               <h4 className="text-2xl font-black text-indigo-900">{item.instrument}</h4>
               <span className="text-[10px] font-black bg-indigo-100 text-indigo-600 px-3 py-1 rounded-full uppercase tracking-widest">{item.category}</span>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
              {item.scores.map((s, si) => (
                <div key={si} className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm transition-all hover:shadow-md">
      ...
      <div onClick={() => setChecked(!checked)} className="flex items-center gap-6 p-8 rounded-[2.5rem] bg-amber-50 border-2 border-amber-100 cursor-pointer select-none transition-all hover:bg-amber-100/50">
        <div className={`w-10 h-10 rounded-2xl border-4 transition-all flex items-center justify-center ${checked ? 'bg-amber-500 border-amber-500' : 'bg-white border-amber-200 shadow-inner'}`}>
        ...
        <p className="text-lg font-black text-amber-950 leading-tight flex-1">
          Confirmo que os dados extraídos coincidem fielmente com as planilhas originais.
        </p>
      </div>

      <button disabled={!checked} onClick={() => onNext(true)} className="w-full h-20 bg-slate-950 disabled:bg-slate-100 disabled:text-slate-300 text-white rounded-[2rem] font-black text-xl transition-all shadow-xl active:scale-[0.98]">
...
const ReportDashboard: React.FC<{ report: CanonicalReport, onReset: () => void }> = ({ report, onReset }) => {
  ...
  const [editedVersions, setEditedVersions] = useState(report.versions);
  ...
  const handleInput = () => {
    if (editorRef.current) {
      ...
      setEditedVersions(prev => ({ ...prev, [activeTab]: content }));
    }
  };

  const processContentForDisplay = (content: string) => {
    // Transforma linhas em MAIÚSCULAS em títulos estilizados
  ...
  const handleAddImage = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file && editorRef.current && hoverY !== null) {
      const reader = new FileReader();
      reader.onload = (ev) => {
        const imgHtml = `<div contenteditable="false" class="image-wrapper my-12 animate-in fade-in zoom-in-95 duration-700"><img src="${ev.target?.result}" class="w-full rounded-2xl border-2 border-slate-100 shadow-sm" alt="Gráfico de avaliação" /><div class="text-[10px] text-center text-slate-400 font-black uppercase mt-4 tracking-[0.3em]">Registro Visual de Desempenho</div></div>`;
  ...
  return (
    <div className="flex flex-col animate-in fade-in slide-in-from-bottom-8 duration-700">
      ...
      {/* Editor Header Card */}
      <div className="bg-white rounded-[2rem] shadow-lg shadow-slate-200/40 p-8 mb-8 flex flex-col md:flex-row justify-between items-center gap-6 print:hidden">
        <div className="flex items-center gap-5">
           <div className="w-3 h-3 bg-indigo-600 rounded-full animate-pulse"></div>
           <div>
              <h3 className="font-black text-slate-800 uppercase tracking-widest text-sm">Relatório em Edição</h3>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Interface Livre de Marcadores</p>
           </div>
        </div>
          ...
          <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200 flex-1 md:flex-none">
            {(['professional', 'simple', 'technical'] as const).map(tab => (
              <button 
                ...
                className={`flex-1 px-5 py-2.5 rounded-lg font-black text-[10px] uppercase tracking-widest transition-all ${activeTab === tab ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
              >
                {tab === 'professional' ? 'Oficial' : tab === 'simple' ? 'Pais' : 'Técnica'}
              </button>
            ))}
          </div>
          <button onClick={() => window.print()} className="bg-slate-900 text-white px-8 py-3 rounded-xl font-black text-xs uppercase tracking-widest flex items-center gap-3 transition-all hover:bg-black active:scale-[0.98] shadow-lg shadow-slate-200">
      ...
      <div ref={containerRef} onMouseMove={onMouseMove} onMouseLeave={() => setHoverY(null)} className="relative bg-white rounded-[3rem] shadow-2xl shadow-slate-200/60 p-12 md:p-24 min-h-[1200px] border border-slate-100 print:shadow-none print:border-none print:p-0">
        
        {/* Formulário de Identificação Integrado */}
        <div className="mb-24 pb-16 border-b-2 border-slate-50 grid grid-cols-1 md:grid-cols-3 gap-12">
          <div className="space-y-3">
            <label className="text-[10px] font-black text-slate-300 uppercase tracking-[0.3em]">Paciente</label>
            <input 
              type="text" 
              placeholder="Nome Completo"
              className="w-full bg-transparent border-none p-0 text-3xl font-black text-slate-900 placeholder:text-slate-100 focus:ring-0"
              value={patient.name} onChange={e => setPatient({...patient, name: e.target.value})}
            />
          </div>
          <div className="space-y-3">
            <label className="text-[10px] font-black text-slate-300 uppercase tracking-[0.3em]">Faixa Etária</label>
            <input 
              type="text" 
              placeholder="Ex: 08 anos"
              className="w-full bg-transparent border-none p-0 text-3xl font-black text-slate-900 placeholder:text-slate-100 focus:ring-0"
              value={patient.age} onChange={e => setPatient({...patient, age: e.target.value})}
            />
          </div>
          <div className="space-y-3">
            <label className="text-[10px] font-black text-slate-300 uppercase tracking-[0.3em]">Emissão</label>
            <input 
              type="text" 
        ...
        {/* Editor WYSIWYG Estilizado */}
        <div 
          ref={editorRef}
          contentEditable
          onInput={handleInput}
          suppressContentEditableWarning
          className="editor-paper outline-none font-serif text-xl md:text-2xl leading-[2.4] text-slate-800 text-justify selection:bg-indigo-100 p-12 -m-12 rounded-[3rem] transition-all"
          dangerouslySetInnerHTML={{ __html: processContentForDisplay(editedVersions[activeTab]) }}
        />

        {/* Inserção de Gráfico Contextual */}
        {hoverY !== null && hoverY > 400 && (
          <div 
            className="absolute left-0 right-0 flex items-center justify-center group pointer-events-none print:hidden transition-all duration-500"
            style={{ top: hoverY, transform: 'translateY(-50%)' }}
          >
             <div className="w-full h-px bg-indigo-50/30 mx-16 group-hover:bg-indigo-100/60 transition-colors"></div>
             <button 
              onClick={() => fileInputRef.current?.click()}
              className="pointer-events-auto flex items-center gap-4 px-10 py-4 bg-white border border-indigo-100 rounded-full shadow-xl shadow-indigo-100/20 text-indigo-600 font-black text-[10px] uppercase tracking-[0.2em] transition-all hover:scale-105 active:scale-95 whitespace-nowrap animate-in fade-in zoom-in-95"
             >
        ...
        <div className="hidden print:block mt-40 border-t pt-16 text-center">
            <div className="w-72 h-px bg-slate-300 mx-auto mb-6"></div>
            <p className="font-black text-slate-900 uppercase tracking-[0.3em] text-sm">Responsável Técnico</p>
            <p className="text-xs text-slate-500 font-bold uppercase tracking-[0.2em] mt-2">{report.context.profession}</p>
        </div>
      </div>
      ...
      <style>{`
        ...
        @media print {
          ...
          .editor-paper { padding: 0 !important; margin: 0 !important; font-size: 13pt !important; line-height: 1.7 !important; color: black !important; }
          .section-header { margin-top: 3rem; margin-bottom: 1.5rem; color: black !important; }
          .section-header::before { background: black !important; }
          input { border: none !important; padding: 0 !important; font-size: 16pt !important; color: black !important; }
        }
